#!/usr/bin/env python3
"""
Count tetramers in DNA sequences.
...
counts 4-mers in each FASTA sequence (no counting across sequence boundaries),

Writes per-run sequence-level 4-mer counts (256 integers per row, no header) to
outputs/tetramer_counts/<cancer_type>/<study_name>/<Run>.csv.xz for downstream analysis
(tetramer frequencies and clustering) but only when the file does not already exist.
...
def _default_paths_from_defaults_yaml(
    ...
    """``(data_dir, fasta_dir, tetramer_counts_dir)`` from ``defaults.yaml`` ``paths``."""
    ...
    return _resolve("data_dir"), _resolve("fasta_dir"), _resolve("tetramer_counts_dir")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    data_dir, fasta_root, counts_root = _default_paths_from_defaults_yaml(repo_root)
    ...
    for csv_path in data_files:
        ...
        seq_output_dir = counts_root / cancer_type / study_name
        ...
        for row in rows:
            ...
            if run_seq_output_path.is_file():
                run_seq_already_exists += 1
                show_run_progress(run_i, progress_total, run, "skipped: seq counts exist")
            ...
            if profile_mode:
                _counts, sequence_rows, err, run_profile = (
                    tetramer_counts_for_run_and_sequences_profiled(fasta_gz)
                )
                study_profile += run_profile
                total_profile += run_profile
            else:
                _counts, sequence_rows, err = tetramer_counts_for_run_and_sequences(fasta_gz)
            if err:
                print(
            ...
            run_seq_written += 1
            study_written += 1
            show_run_progress(run_i, progress_total, run, "wrote seq counts")
            if first_run_per_study:
                break
